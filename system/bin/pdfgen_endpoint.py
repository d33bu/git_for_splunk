...
class PDFGenHandler(splunk.rest.BaseRestHandler):
    ...
    _useChromiumLegacyExport = False
    ...
    ARG_SAVE_SNAPSHOT = 'save-snapshot'
    ...
    WEB_FEATURES_ENTITY = '/configs/conf-web-features'
    ...
    def _handleConverter(self):
        convertedData = convertDashboardToPdfContent(self.sessionKey, self._namespace, self._owner, self._dashboardContent, self._dashboardName, self._studioErrors, self._chromiumTimeoutDuration, self._chromiumScreenshotDelay, self._fileFormat.upper(), self._saveSnapshot)
    ...
    def _handleStudioDashboardPdf(self):
        try:
            if self._saveSnapshot:
                self._snapshotData = self._handleConverter()
            else:
    ...
    def _initialize(self):
        try:
            ...
            exporter = 'Chromium' if self._useChromiumLegacyExport else 'NodeJS'
            ...
            if self._viewType == self.VIEW_TYPE_DASHBOARD:
                ...
                self._views.append(pv.Report(savedSearchName=self._reportName, exporter=exporter, namespace=self._namespace, owner=self._owner, sessionKey=self.sessionKey))
                self._title = self._reportName
            elif self._viewType == self.VIEW_TYPE_SEARCH:
                self._title = self._reportName
                self._views.append(pv.SearchReport(search=self._searchStr, et=self._et, lt=self._lt, title=self._title, exporter=exporter, namespace=self._namespace, owner=self._owner, sessionKey=self.sessionKey))
    ...
    def _respond(self):
        ...
        if self._saveSnapshot and self._snapshotData:
            self._snapshotResponse()
        else:
            try:
    ...
    def _snapshotResponse(self):
        try:
            self.response.setHeader('content-type', 'application/json')
            self.response.write(json.dumps(self._snapshotData))
            self.response.setStatus(self._snapshotData.get('status'))
        except Exception as e:
            errorMsg = "Exception raised while trying to respond. Unable to take snapshot of %s: %s" % (self._dashboardName, str(e))
            pu.logErrorAndTrace(errorMsg)
    ...
    def _initParameters(self):
        ...
        if fileFormatArg == 'png' and self._viewType == self.VIEW_TYPE_DASHBOARD and self._dashboardContent != None:
            ...
            errorMsg = "Only pdf and png types are supported."
            self._outputError([errorMsg])
            raise ArgError(errorMsg)
        
        self._saveSnapshot = self.args.get(self.ARG_SAVE_SNAPSHOT) or self._saveSnapshot
        ...
        if self.ARG_INPUT_PAPERSIZE in self.args:
            ...
            if paperSizeArg in pdfrenderer.PAPERSIZES:
                ...
                logger.warning('Invalid paper size "%s"' % paperSizeArg)
        ...
        if self.ARG_INPUT_MAX_ROWS_PER_TABLE in self.args:
            ...
            try:
                ...
                logger.warning('Max-rows-per-table="%s" is invalid, must be an integer' % maxRowsPerTableArg)
        ...
        if self.ARG_INPUT_RESULT_ID in self.args:
            ...
            if resultIdStr.isdigit():
                ...
                logger.warning("pdf-init received invalid per_alert_result_id=%s"% resultIdStr)
        ...
        for arg in self.args:
            ...
            if arg not in self._validArgs and arg not in pdfrenderer.ALL_PDF_SETTINGS:
                ...
                if self.sidRE.match(arg) is None:            
                    logger.warning("Invalid argument passed to pdfgen/render. arg=%s value=%s" % (arg, self.args[arg]))
    ...
    def _initLimitsDefaults(self):
        ...
        try:
            # SPL-107168 Passing namespace and owner to generate context specifc endpoint
            limitsSettings = entity.getEntity(self.LIMITS_ENTITY, 'pdf', namespace=self._namespace, owner=self._owner, sessionKey=self.sessionKey)
            logger.debug("limitsPDFStanza=%s" % (str(limitsSettings)))
            self._maxRowsPerTable = int(limitsSettings.get('max_rows_per_table', self._maxRowsPerTable))
            self._timeoutDuration = int(limitsSettings.get('render_endpoint_timeout', self._timeoutDuration))
            self._chromiumTimeoutDuration = int(limitsSettings.get('render_chromium_timeout', self._chromiumTimeoutDuration))
            self._chromiumScreenshotDelay = int(limitsSettings.get('render_chromium_screenshot_delay', self._chromiumScreenshotDelay))
        except Exception as e:
            logger.error("Could not access or parse pdf stanza of limits.conf. Error=%s" % str(e))

        try:
            webFeaturesSettings = entity.getEntity(self.WEB_FEATURES_ENTITY, 'feature:pdfgen', owner=self._owner, sessionKey=self.sessionKey)
            logger.debug("webFeaturesPdfgenStanza=%s" % (str(webFeaturesSettings)))
            self._useChromiumLegacyExport = normalizeBoolean(webFeaturesSettings.get('activate_chromium_legacy_export', self._useChromiumLegacyExport))
        except Exception as e:
            logger.error("Could not access or parse feature:pdfgen stanza of web-features.conf. Error=%s" % str(e))
    ...
    def _renderTable(self, pdfRenderer, view):
        ...
        if self._resultId >= len(list(results)):
            logger.warning('PDFRenderTable: found result_id=%d is greater than size_of_results=%d, PDF will be generated with all of search results.'
    ...
    def _renderList(self, pdfRenderer, view):
        ...
        if self._resultId >= len(list(results)):
            logger.warning('PDFGenHandler::_renderList: found result_id=%d is greater than size_of_results=%d, PDF will be generated with all of search results.'
                        % (self._resultId, len(list(results))))
    ...
    def _renderSvgBasedViz(self, pdfRenderer, view, props, mode, postSearch=None):
        ...
        if mode == "chart":
            ...
            svgBasedViz = pc.Chart(self._dashboardName, data, fields, props, self._locale, self._useChromiumLegacyExport, server_zoneinfo=self._server_zoneinfo)
        elif mode == "map":
            svgBasedViz = pc.Map(self._dashboardName, data, fields, props, self._locale, self._useChromiumLegacyExport, server_zoneinfo=self._server_zoneinfo)
        elif mode == "single":
            svgBasedViz = pc.Single(self._dashboardName, data, fields, props, self._locale, self._useChromiumLegacyExport, server_zoneinfo=self._server_zoneinfo)
    ...
    def _renderEvents(self, pdfRenderer, view):
        ...
        if self._resultId >= len(list(events)):
            logger.warning('PDFrenderEvents: found result_id=%d is greater than size_of_events=%d, PDF will be generated with all of search events.'
