...
def get_conf_files(files):
    cur_dir = get_app_path(os.path.abspath(__file__))
    all_files = []
    all_confs = [ggc.myta_global_settings_conf, ggc.myta_cred_conf] + files
    for f in all_confs:
        all_files.append(os.path.join(cur_dir, "local", f))
    return all_files
...
def get_config_object(server_uri, session_key, app_name=ggc.splunk_ta_google):
    """Get the splunksdc.config.ConfManager object for the provided Splunk app name.

    Args:
        server_uri (str): Splunk server uri
        session_key (str): Splunk session key
        app_name (str, optional): Splunk app name. Defaults to ggc.splunk_ta_google.

    Returns:
        splunksdc.config.ConfManager: ConfManager object for the specified Splunk app.
    """
