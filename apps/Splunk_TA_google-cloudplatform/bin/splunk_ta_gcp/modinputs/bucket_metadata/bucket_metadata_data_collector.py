...
class BucketMetadataCollector:
    ...
    def get_service_object(self, config, profile):
        ...
        proxy_config = {}
        if proxy:
            logger.debug(f"Proxy is enabled.")
            ...
            proxy_config = self.get_proxy_object(self._settings.proxy)
        http = tacommon.authorise_http_credential(proxy_config, credentials)
        ...
        try:
            ...
            if sourcetype == "google:gcp:buckets:csvdata":
                ...
                response = decoder.decode(
                    response_chunk.read(),
                    final=ingestion_status.get("completed", False),
                )
