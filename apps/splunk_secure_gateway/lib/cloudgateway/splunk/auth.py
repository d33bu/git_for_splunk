...
class SplunkJWTCredentials(UserAuthCredentials):
    ...
    def create_jwt_token(self,
                         ...
                         jwt_expiration=constants.EXPIRES_ON):

        data = self.jwt_token_data(self.username, audience, jwt_expiration)

        r, content = splunk_client.create_jwt_token(session_token, data)
        if r.status != HTTPStatus.CREATED:
            raise Exception('Exception creating JWT token with code={}, message={}'.format(r.status, content))

        response_json = json.loads(content)
        self.token = response_json['entry'][0]['content']['token']
        self.token_id = response_json['entry'][0]['content']['id']
