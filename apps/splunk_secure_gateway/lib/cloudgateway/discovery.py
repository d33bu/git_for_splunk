...
from spacebridge_protocol.discovery_pb2 import (
    ...
    InitiateHashCodeInstanceRequest
)
...
def initiate_hashcode(hashed_auth: bytes,
                      encryption_context: EncryptionContext,
                      config: SplunkConfig = SplunkConfig()) -> Response:
    """
    Initiates a discovery registration by uploading the hashcode the client will use.
    Should be used as a part of client auth-code registration process.
    """
    request = InitiateHashCodeInstanceRequest(
        hashCode=hashed_auth
    )
    try:
        response = requests.post(
            url=f'https://{config.get_spacebridge_discovery_server()}/{DISCOVERY_PATH}/hashcodes/initiate',
            headers={
                HEADER_SPACEBRIDGE_CLIENT_ID: encryption_context.sign_public_key(transform=encryption_context.generichash_hex),
            },
        ...
        raise RegistrationError(prefix='HTTP Timeout while sending new hashcode to Spacebridge Discovery',
                                code=HTTPStatus.REQUEST_TIMEOUT,
                                message=str(e))

    if response.status_code != HTTPStatus.OK:
        raise RegistrationError(prefix='HTTP error while Initiating a new hashcode at Spacebridge Discovery',
                                code=response.status_code,
                                message=response.text)
...
def update_hashcode_instance(hashed_auth: bytes,
                             instance_id: str,
                             encryption_context: EncryptionContext,
                             config: SplunkConfig = SplunkConfig()) -> Response:
    """
    Updates the instance metadata for an ephemeral hashcode.
    Must be first initiated by the registering client.
    """
