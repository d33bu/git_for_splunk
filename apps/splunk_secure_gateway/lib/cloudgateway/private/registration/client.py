...
from cloudgateway.private.util.constants import HEADER_SPACEBRIDGE_CLIENT_ID, HEADER_CONTENT_TYPE
...
def make_device_authentication_request(device_info, encryption_context, config, key_bundle=None,
    ...
    with requests_ssl_context(key_bundle) as cert:
        ...
        try:
            spacebridge_header = {
                HEADER_SPACEBRIDGE_CLIENT_ID: sb_auth_header(encryption_context),
            }
...
def parse_credentials_bundle(encrypted_credentials_bundle, encryption_context):
    ...
    meta_data_base64 = base64.b64encode(credentials_bundle.environmentMetadata.serializedMetadata).decode('ascii')
    env_metadata = EnvironmentMetadata(meta_data_base64,
    ...
    with requests_ssl_context(pkcs12) as cert:
        try:
            return requests.post(
                ...
                headers={
                    HEADER_CONTENT_TYPE: 'application/x-protobuf',
                    HEADER_SPACEBRIDGE_CLIENT_ID: sb_auth_header(encryption_context)
                },
                data=request_proto.SerializeToString(),
                proxies=config.get_proxies(),
