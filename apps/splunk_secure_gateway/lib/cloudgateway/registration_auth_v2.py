...
def query_registration_confirmation(auth_id: bytes,
    ...
    response = requests.post(
        ...
        headers={
            HEADER_SPACEBRIDGE_CLIENT_ID: encryption_context.sign_public_key(transform=encryption_context.generichash_hex),
...
def query_client_registration(config: SplunkConfig,
    ...
    response = _query_client_registration_impl(auth_id, client_id, config, num_retries)
...
def _query_client_registration_impl(auth_id: bytes, client_id: str, config: SplunkConfig, num_retries: int = 0):
    ...
    url = f'https://{config.get_spacebridge_server()}/{REGISTRATION_V2_PATH}/registrar_query'
    headers = {
        HEADER_SPACEBRIDGE_CLIENT_ID: client_id,
        HEADER_SPACEBRIDGE_AUTH_ID: auth_id.hex()
    }

    # List of status codes for which retries should occur
    retry_status_list = [404, 500, 502, 503, 504]
    ...
    for attempt in range(num_retries + 1):
        try:
            ...
            # If the status code is in the retryable list, raise a RequestException to trigger a retry
            if response.status_code in retry_status_list:
                raise requests.RequestException(f"Retryable HTTP status code: {response.status_code}")

            # If the status code is not OK and not in retryable list, raise a RegistrationError immediately
            raise RegistrationError(prefix='HTTP error during RegistrarQueryRequest',
                                    code=response.status_code,
                                    message=response.text)
