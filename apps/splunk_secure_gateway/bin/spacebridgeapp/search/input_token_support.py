...
_INPUT_TOKEN_PATTERN = re.compile(r'\$+([\w |.-]+)\$+')
_INPUT_TOKEN_REPLACE = re.compile(r'\$+[\w |.-]+\$+')
...
def inject_tokens_into_string(input_tokens, query, default=None):
    ...
    while len(remaining_query) > 0:
        ...
        if result:
            ...
            if token in input_tokens.keys() and is_valid_token_value(input_tokens[token]):
...
def is_valid_token_value(token):
    """
    Check if input token's value is valid.
    """
