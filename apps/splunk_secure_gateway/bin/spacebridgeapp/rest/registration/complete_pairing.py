...
from spacebridgeapp.rest.registration.registration_app import RegistrationApp
...
from spacebridgeapp.util.constants import DEVICE_NAME, DEVICE_TYPE, DEVICE_ID, APP_ID, \
    ...
    REGISTRATION_TYPE, REGISTRATION_METHOD, AUTH_METHOD, DEVICE_MANAGEMENT_METHOD, TRUE, FALSE, APP
...
def handle_confirmation(config: SecureGatewayConfig, log: logging.Logger,
    ...
    is_tokens_enabled = get_tokens_enabled(log, system_authtoken)
    register_with_jwt = saml_register or self_register or is_tokens_enabled
    ...
    if register_with_jwt:
        if not is_valid_session_token(user, session_token):
            return jwt_error_response(user)
        if not is_tokens_enabled:
            return token_disabled_error_response()
        try:
            app = extract_parameter_if_exists(body, APP, QUERY, RegistrationApp.NOT_SELECTED.value)
            is_edge_hub = RegistrationApp.EDGE.value == app
            credentials = generate_jwt_token(config, log, user, system_authtoken, is_edge_hub)
