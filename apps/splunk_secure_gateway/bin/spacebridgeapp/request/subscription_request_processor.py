...
from spacebridgeapp.dashboard import dashboard_helpers
...
from spacebridgeapp.subscriptions.subscription_search_requests import create_job_from_search, get_sid_from_ref
...
async def _spawn_search_job(log: logging.Logger, request_context, async_splunk_client, subscription_search,
                            input_tokens, sid):
...
async def process_subscribe_dashboard_input_search_request(log: logging.Logger,
    ...
    async_secret_management_client = async_client_factory.secret_management_client()
    ...
    subscription_id = await create_subscription(log,
                                                ...
                                                async_splunk_client=async_splunk_client,
                                                async_kvstore_client=async_kvstore_client,
                                                async_secret_management_client=async_secret_management_client,
...
async def process_subscribe_saved_search_request(log: logging.Logger,
    ...
    if trellis_enabled:
        ...
        if not trellis_split_by:
            log.debug(
                "SavedSearch: Trellis split by field not provided by client device, calculating trellis split by field")
...
async def process_subscribe_udf_datasource(log: logging.Logger,
    ...
    search_defn = create_search_from_data_source(udf_data_source=udf_data_source,
                                                 udf_dashboard_definition=dashboard_description.definition)
...
def _get_base_search(dashboard_definition, base_id):
    ...
    if isinstance(dashboard_definition, UdfDashboardDescription):
        ...
        return create_search_from_data_source(
            udf_data_source=udf_data_source,
            udf_dashboard_definition=dashboard_definition
        )
...
async def lazy_load_subscription_search(log: logging.Logger,
    ...
    # Lazy-loading must follow the same algorithms as `create_job_from_search` for sid generation, otherwise it might have different sid
...
async def _create_subscription_credential(request_context, subscription_credential, async_secret_management_client):
    system_auth_header = request_context.system_auth_header
    credential = await async_secret_management_client.fetch_sensitive_data(user_name=request_context.current_user,
                                                                           system_auth_header=system_auth_header)
    if not credential:
        await async_secret_management_client.create_sensitive_data(user_name=request_context.current_user,
                                                                   data=subscription_credential,
                                                                   system_auth_header=system_auth_header)
    else:
        await async_secret_management_client.update_sensitive_data(user_name=request_context.current_user,
                                                                   data=subscription_credential,
                                                                   system_auth_header=system_auth_header)
...
async def create_subscription(log: logging.Logger,
                              ...
                              async_splunk_client,
                              async_kvstore_client,
                              async_secret_management_client,
                              visualization_id,
                              id_gen=uuid4
                              ):
    """
    ...
    subscription_credential = SubscriptionCredential(subscription_id=subscription_id,
                                                     session_key=session_key,
                                                     session_type=session_type,
                                                     shard_id=request_context.shard_id,
                                                     last_update_time=now,
                                                     _user=request_context.current_user)

    await _create_subscription_credential(request_context, subscription_credential, async_secret_management_client)
    await _create_subscription(log, request_context, subscription, async_kvstore_client),
