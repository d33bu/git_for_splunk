...
class CollectorAborter:
    SIGNALS_HANDLED = [signal.SIGINT, signal.SIGTERM, signal.SIGALRM]
    ...
    def __enter__(self) -> None:
        ...
        for sig in self.__class__.SIGNALS_HANDLED:
            signal.signal(sig, self.handler) # type: ignore
...
class Task(Serializable):
    ...
    class RunInfo(Serializable, CollectorStateObserver): # pylint: disable=too-many-public-methods
        ...
        def archive(self) -> None: # pylint: disable=too-many-locals
            ...
            with tarfile.open(tarball_name, 'w:gz', format=tarfile.GNU_FORMAT) as tar:
                ...
                for file in files:
                    ...
                    if file.startswith('diag_') and file.endswith('.tar.gz'):
                        with tarfile.open(full_path, 'r:gz', format=tarfile.GNU_FORMAT) as src_tar:
                            try:
