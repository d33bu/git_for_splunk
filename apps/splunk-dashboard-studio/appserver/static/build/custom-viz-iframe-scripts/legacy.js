...
var require_cookie = __commonJS({
  "../../node_modules/@splunk/ui-utils/cookie.js"(exports) {
    "use strict";
...
var require_config = __commonJS({
  "../../node_modules/@splunk/splunk-utils/config.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", {
      value: true
    });
    ...
    var root = typeof window === "undefined" ? self : window;
...
var require_url = __commonJS({
  "../../node_modules/@splunk/splunk-utils/url.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", {
      value: true
    });
    ...
    var config = _interopRequireWildcard(require_config());
    ...
    function withConfig(_ref) {
      ...
      function createURL3(pathInput, queryParams) {
      ...
      function createStaticURL2(path) {
        ...
        return createURL3.apply(void 0, ["static/".concat(path)].concat(rest));
      }
      ...
      function createDocsURLFromParams(params, configOptions) {
        return createURL3("/help", params, configOptions);
      }
    ...
    var createAppDocsURL = _withConfig.createAppDocsURL;
    var createDocsURL = _withConfig.createDocsURL;
    var createRESTURL = _withConfig.createRESTURL;
    var createStaticURL = _withConfig.createStaticURL;
    var createURL2 = _withConfig.createURL;
    var insertCacheBuster = _withConfig.insertCacheBuster;
    exports.insertCacheBuster = insertCacheBuster;
    exports.createURL = createURL2;
...
var require_DataSet = __commonJS({
  "../../node_modules/@splunk/datasource-utils/DataSet.js"(exports, module2) {
    "use strict";
    ...
    module2.exports = __toCommonJS(DataSet_exports);
    var lodash_exports = {};
...
async function getCSSRules(styleSheets, options) {
  ...
  await Promise.all(styleSheets.map(async (sheet) => {
    if ("cssRules" in sheet) {
      try {
        if (sheet.href) {
          const css = await fetchCSS(sheet.href);
  ...
  return Promise.all(deferreds).then(async () => {
    await Promise.all(styleSheets.map(async (sheet) => {
      if ("cssRules" in sheet) {
        try {
          if (sheet.href) {
...
function handleMessage(event) {
  ...
  const handlersSet = messageHandlers.get(type) ?? /* @__PURE__ */ new Set();
  let handled = false;
  for (const handler of handlersSet) {
    if (event.source === handler.iframeRef?.current?.contentWindow || handler.iframeRef === void 0) {
      handled = true;
      handler(event);
    }
  }
...
function registerMessageHandler(type, handler, iframeRef) {
  handler.iframeRef = iframeRef;
  const handlersSet = messageHandlers.get(type) ?? /* @__PURE__ */ new Set();
  handlersSet.add(handler);
  messageHandlers.set(type, handlersSet);
}
...
async function exportAsPng(event) {
  const node = getRootElement();
  try {
    const dataUri = await toPng(node);
    const message = {
      type: "CONTENT-EXPORTER-SUCCESS" /* Success */,
      id: event.data.id,
      dataUri
    };
    ...
    const errorMessage = {
      type: "CONTENT-EXPORTER-ERROR" /* Error */,
      id: event.data.id,
      error: error.message ?? "Unknown error"
    };
    window.parent.postMessage(errorMessage, getParentOrigin());
  }
}
function initContentExporter() {
  registerMessageHandler("CONTENT-EXPORTER-REQUEST" /* Request */, exportAsPng);
}

// src/FetchHandler/IframeFetchHandler.ts
var pendingFetchRequests = /* @__PURE__ */ new Map();
function handleFetchError(event) {
  const { requestId } = event.data;
  const request = pendingFetchRequests.get(requestId);
  if (!request) {
    return;
  }
...
function handleFetchResponse(event) {
  ...
  pendingFetchRequests.delete(requestId);
  const { arrayBuffer, init } = event.data;
  const response = new Response(arrayBuffer, init);
  request.resolve(response);
}
function initIframeFetchSetup() {
  const customWindow = window;
  let requestId = 0;
  customWindow.originalFetch = window.fetch;
  const parentOrigin = getParentOrigin();
  customWindow.fetch = (url, options = {}) => {
    const absoluteUrl = new URL(url, parentOrigin);
    if (absoluteUrl.origin === parentOrigin) {
      return new Promise((resolve, reject) => {
        requestId++;
        pendingFetchRequests.set(requestId, { resolve, reject });
        window.parent.postMessage(
          {
            type: "FETCH-PROXY-REQUEST" /* FetchProxyRequest */,
            requestId,
    ...
    return customWindow.originalFetch(url, options);
  };
  registerMessageHandler("FETCH-PROXY-RESPONSE" /* FetchProxyResponse */, handleFetchResponse);
...
var initIframeFonts = async () => {
  ...
  const resolvedFonts = {};
  await Promise.all(
    Array.from(fontMatches).map(async (match) => {
  ...
  const bootstrapCSSWithFontsLoaded = bootstrapCSSText.replace(urlRegex, (match, fontUrl) => resolvedFonts[fontUrl] ?? match).trim();
  const sheet = new CSSStyleSheet();
  sheet.replaceSync(bootstrapCSSWithFontsLoaded);
  document.adoptedStyleSheets = [sheet];
};

// ../enterprise-dashboard-service/src/visualizations/constants.ts
var DEFAULT_DATA_PARAMS = Object.freeze({
  count: 1e3,
  offset: 0,
  sort_direction: "desc",
  show_metadata: true
...
// src/DataParamsHandler/IframeDataParamsHandler.ts
function handleUpdateDataParams(dataSourceKey, dataParams) {
  const message = {
    type: "UPDATE-DATA-PARAMS" /* UpdateDataParams */,
    dataSourceKey,
    dataParams
  };
  window.parent.postMessage(message, getParentOrigin());
...
// src/Iframe/state.ts
var [getIframeState, setIframeState] = /* @__PURE__ */ (() => {
  const state = {};
...
var handleError = (error) => {
  const message = {
    type: "HANDLE-GLOBAL-ERROR" /* Handle */,
    error: {
  ...
  setIframeState("error", message);
  window.parent.postMessage(message, getParentOrigin());
};
var clearGlobalError = () => {
  setIframeState("error", null);
  window.parent.postMessage({ type: "CLEAR-GLOBAL-ERROR" /* Clear */ }, getParentOrigin());
};
var isValidProp = (obj, prop) => {
  return prop in obj;
};
var ErrorHandlerFunctions = ["updateView", "setupView", "reflow", "formatData"];
var addErrorHandlingToViz = (viz) => {
  return new Proxy(viz, {
    get(target, prop, receiver) {
      const originalProp = Reflect.get(target, prop, receiver);
      if (!isValidProp(target, prop) || !ErrorHandlerFunctions.includes(prop)) {
        return originalProp;
      }
      return async (...args) => {
        try {
          return await originalProp.bind(target)(...args);
        } catch (error) {
          handleError(error);
          throw error;
        }
...
// src/DrilldownHandler/IframeDrilldownHandler.ts
function handleIframeDrilldown(drilldownArgs) {
  const message = {
    type: "HANDLE-DRILLDOWN" /* HandleDrilldown */,
    ...drilldownArgs,
    originalEvent: { type: drilldownArgs.originalEvent?.type ?? "click" }
  };
  window.parent.postMessage(message, getParentOrigin());
}

// src/IframeUtils/drilldown.ts
var ISO_REGEX = /^\d\d\d\d-\d\d?-\d\d?T\d\d?:\d\d?(:\d\d?)?(\.\d\d?\d?)?([+-]\d\d?:\d\d|Z)?$/;
var toEpochSeconds = (timeValue) => {
  ...
  const trimmedString = timeValue.trim();
  const isIsoString = trimmedString.match(ISO_REGEX);
  if (isIsoString) {
    return new Date(trimmedString).getTime() / 1e3;
  }
  const parsedFloat = Number.parseFloat(trimmedString);
...
var modifyTimeRangeDrilldownInfo = (transformedPayload) => {
  if (!("earliest" in transformedPayload)) {
    ...
    transformedPayload.latest = void 0;
    return false;
  }
  const earliestEpochSeconds = toEpochSeconds(transformedPayload.earliest);
  transformedPayload.earliest = earliestEpochSeconds;
  transformedPayload.name = "_time";
  transformedPayload.value = earliestEpochSeconds;
  transformedPayload["row._time.value"] = earliestEpochSeconds;
  if (transformedPayload.latest) {
    const latestEpochSeconds = toEpochSeconds(transformedPayload.latest);
    transformedPayload.latest = latestEpochSeconds;
    const span = latestEpochSeconds - earliestEpochSeconds;
    transformedPayload["row._span.value"] = span;
    transformedPayload._span = span;
  } else {
    transformedPayload.latest = void 0;
  }
  return true;
};
var modifyFieldValueDrilldownInfo = (transformedPayload) => {
  if (typeof transformedPayload.data !== "object" || !transformedPayload.data || !Object.keys(transformedPayload.data).length) {
    return false;
  }
...
var drilldown = (_viz, payload, originalEvent) => {
  ...
  if (payload.action === "fieldvalue") {
    isValidDrilldown = modifyFieldValueDrilldownInfo(transformedPayload) || isValidDrilldown;
  }
  if (isValidDrilldown) {
    handleIframeDrilldown({ action: studioEvent, payload: transformedPayload, originalEvent });
  }
};
...
var triggers = {
  ...
  invalidateFormatData: async (viz) => {
    const dataSources = getIframeState("dataSources");
    const options = viz.getCurrentConfig();
    const data = viz.combineData(dataSources, options);
    const formattedData = await viz.formatData(data, options);
    viz.setCurrentData(formattedData);
    return viz.updateView(formattedData, options);
  },
  invalidateUpdateView: (viz) => {
    const data = viz.getCurrentData();
    const options = viz.getCurrentConfig();
    return viz.updateView(data, options);
  },
  updateDataParams: (_, dataParams) => {
    const validParams = {};
    ["count", "offset", "show_metadata"].forEach((key) => {
      if (key in dataParams) {
        validParams[key] = dataParams[key];
      }
    });
    if ("sort_key" in dataParams && typeof dataParams.sort_key === "string") {
      validParams.sort = {
        [dataParams.sort_key]: dataParams.sort_direction ?? DEFAULT_DATA_PARAMS.sort_direction
      };
    }
    handleUpdateDataParams("primary", validParams);
  },
  drilldown: drilldown_default
};
function modifyTriggers(viz) {
  const originalTriggerFunc = viz.trigger;
  viz.trigger = (event, ...args) => {
    if (event in triggers) {
      const trigger = triggers[event];
      return trigger(viz, ...args);
    }
    return originalTriggerFunc(event, ...args);
  };
}
var onRequireJSLoad = async (CustomVizClass, appName, vizName) => {
  const rootElement = getRootElement();
  const CustomVizClassDef = CustomVizClass;
  const originalSxmlCustomViz = new CustomVizClassDef(rootElement, appName, vizName);
  modifyTriggers(originalSxmlCustomViz);
  const sxmlCustomViz = addErrorHandlingToViz(originalSxmlCustomViz);
  setIframeState("sxmlCustomViz", sxmlCustomViz);
  await sxmlCustomViz?.setupView?.();
  window.parent.postMessage(
    {
      type: "IFRAME-INIT" /* Init */
    },
    getParentOrigin()
  );
};
var initLegacyCustomViz = async () => {
  const rootElement = getRootElement();
...
// src/MessageHandler/IframeMessageHandler.ts
...
function initIframeMessageHandler() {
  return window.addEventListener("message", handleIframeMessage);
}

// src/IframeUtils/DataTransformer.ts
...
var formatData = (dataSource, outputMode) => {
  ...
  switch (outputMode) {
    ...
    case "json_rows" /* ROW_MAJOR_OUTPUT_MODE */:
      return dataSet.toJSONRows();
    case "json" /* RAW_OUTPUT_MODE */:
      return dataSet.toJSONArray();
    default:
      throw new Error(`Unknown output mode: ${outputMode}`);
  }
};
...
// src/PropsHandler/IframePropsHandler.ts
var localProps = {};
async function handlePropsUpdate(event) {
  const vizType = getRootElement()?.getAttribute("data-viz-type");
  const [appName, vizName] = vizType.split(".");
  const { props } = event.data;
  if (props.dataSources) {
    localProps.dataSources = props.dataSources;
  }
  if (props.options) {
    localProps.options = props.options;
  }
  const { dataSources: dataSourcesFromUDF, options } = localProps;
  const sxmlCustomViz = getIframeState("sxmlCustomViz");
  if (!dataSourcesFromUDF || !sxmlCustomViz) {
    return;
  }
  if (!dataSourcesFromUDF?.primary?.data?.columns) {
    return;
  }
  if (getIframeState("error")) {
    clearGlobalError();
  }
  const formattedOptions = {};
  Object.entries(options ?? {}).forEach(([key, value]) => {
    const keyWithoutNamespace = key.replace(`${appName}.${vizName}.`, "");
    const newKey = `${sxmlCustomViz?.getPropertyNamespaceInfo().propertyNamespace}${keyWithoutNamespace}`;
    formattedOptions[newKey] = value;
  });
  if (props.hasEventHandlers) {
    formattedOptions["display.visualizations.custom.drilldown"] = "all";
  }
  sxmlCustomViz?.setCurrentConfig(formattedOptions);
  const dataSources = transformData(sxmlCustomViz, dataSourcesFromUDF);
  setIframeState("dataSources", dataSources);
  const dataToFormat = sxmlCustomViz?.combineData(dataSources, formattedOptions) ?? {};
  const formattedData = await sxmlCustomViz?.formatData(dataToFormat, formattedOptions) ?? {};
  sxmlCustomViz?.setCurrentData(formattedData);
  return sxmlCustomViz?.updateView(formattedData, formattedOptions);
}
function intiIframePropsSetup() {
  registerMessageHandler("PROPS-UPDATE" /* PropsUpdate */, handlePropsUpdate);
}

// src/ResizeHandler/IframeResizeHandler.ts
function handleResize(_event) {
  const data = getIframeState("dataSources");
  if (!data) {
    return;
  }
  const sxmlCustomViz = getIframeState("sxmlCustomViz");
  sxmlCustomViz?.reflow();
}
function initIframeResizeSetup() {
