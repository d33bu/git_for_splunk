...
class Workday(object):
    ...
    REST_API_ENDPOINT_PATTERN = re.compile("^https://(?P<host>[^/:]+)(?::\d+)?/ccx/(api|api/privacy)/v1/(?P<tenant>[^/]+)$")
    ...
    #Match a correctly formatted custom report url in Workday
    SIGNON_REPORT_PATTERN = re.compile("^https://(?P<host>[^/:]+)(?::\d+)?/ccx/service/customreport2/(?P<tenant>[^/]+)/(?P<user>[^/]+)/(?P<report>[^/]+)$")

    # Match a FQDN that is owned by Workday and may host a valid tenant
    WORKDAY_HOSTNAME_PATTERN = re.compile("^.+\.(?:workday\.com|myworkday\.com|workdaysuv\.com|workday\.net|workdaygov\.com|myworkdaygov\.com)$")
    ...
    def __init__(self, rest_api_endpoint, token_endpoint, client_id, client_secret, refresh_token, http_user_agent=None,
                 proxies=None, helper=None):
        """
        ...
        Args:
            ...
            proxies: dictionary of proxy urls to use for all requests, same format as expected by python-requests
        ...
        try:
            ...
            raise ValueError("Invalid URL endpoint provided: {}".format(str(error)))
    ...
    def get(self, endpoint, *args, **kwargs):
        ...
        while True:
            try:
                match = self.SIGNON_REPORT_PATTERN.match(endpoint)
                if match:
                    url = endpoint
                else:
                ...
                if retry_count < self.MAX_RETRIES:
                    ...
                    if self._helper:
                        self._helper.log_warning(
                            "Request timed out, retrying (attempt {}/{})".format(retry_count, self.MAX_RETRIES))
                    continue
                ...
                if retry_count < self.MAX_RETRIES:
                    ...
                    if error.response.status_code == 401:
                        self._refresh_access_token()
                    if self._helper:
                        self._helper.log_warning("Request failed with error code ({}), retrying (attempt {}/{})".format(
                            error.response.status_code, retry_count, self.MAX_RETRIES))
                    continue
                # Halt and escalate, retries exhausted
                else:
                    raise
