...
class UpgradeClusterStandaloneRestEndpoint(PersistentServerConnectionApplication):
    def __init__(
        self,
    ...
    def handle(self, in_string: bytes) -> JsonObject:
        return UpgradeClusterStandaloneRestProcessor(SingleInstanceType.STANDALONE).handle(
            in_string
        )
