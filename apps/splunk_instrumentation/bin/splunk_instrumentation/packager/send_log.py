...
class SendLog(object):
    ...
    def send_attempted(self, start, end, batchNum, visibility, method=METHOD['AUTO'], time_range=None, count=None):
        """send_attempted.
        ...
        """
        logging.info(
            "attempt send " + date_to_timestamp_str(start) + ' to ' + date_to_timestamp_str(end) + ' for batch ' + str(batchNum))
        self._submit_status(
            self._status["ATTEMPTED"], start, end, batchNum, visibility,
            source=self.source, method=method, time_range=time_range, count=count)

    def send_completed(self, start, end, batchNum, visibility, method=METHOD['AUTO'], time_range=None, count=None):
        """send_completed.
        ...
        """
        logging.info(
            "completed send " + date_to_timestamp_str(start) + ' to ' + date_to_timestamp_str(end) + ' for batchNum ' + str(batchNum))
        self._submit_status(
            self._status["SUCCESS"], start, end, batchNum, visibility,
            source=self.source, method=method, time_range=time_range, count=count)

    def send_failed(self, start, end, batchNum, visibility, method=METHOD['AUTO'], time_range=None, count=None):
        """send_failed.
        ...
        """
        logging.info(
            "failed send " + date_to_timestamp_str(start) + ' to ' + date_to_timestamp_str(end) + ' for batchNum ' + str(batchNum))
        self._submit_status(
            self._status["FAILED"], start, end, batchNum, visibility,
    ...
    def _submit_status(
            self, status, start, end, batchNum, visibility, source=None, method=METHOD['AUTO'], time_range=None, count=None):
        """ submit_status.
        ...
        data = self.bundle_data(status, start, end, batchNum, visibility, source, method, time_range, count)
