...
def getEmailList(email):
    ...
    for email_item in tmpList:
        if re.search(sec.EMAIL_FORMAT, email_item):
            emailList.append(email_item.strip().lower())
        else:
...
def setEmailRecipientField(email, field, recipientList):
    stripped = ','.join([str(elem) for elem in recipientList])
    del email[field]
    if recipientList:
        email.add_header(field, stripped)

def checkRecipientAllowed(recipient, allowedDomains, ssContent):
...
def mail(sessionKey, email, argvals, ssContent, sid):
    ...
    if email['To']:
        toRecipients = getEmailList(email['To'])
    if email['Cc']:
        ccRecipients = getEmailList(email['Cc'])
    if email['Bcc']:
        bccRecipients = getEmailList(email['Bcc'])
    ...
    if ssContent.get('action.email.allowedDomainList') != "" and ssContent.get('action.email.allowedDomainList') != None:
        ...
        if domainsRestriction == sec.ALLOWED_DOMAIN_LIST_ALLOW_ALL:
            ...
            toRecipients = [recipient for recipient in toRecipients if checkRecipientAllowed(recipient, domains, ssContent)]
            ccRecipients = [recipient for recipient in ccRecipients if checkRecipientAllowed(recipient, domains, ssContent)]
            bccRecipients = [recipient for recipient in bccRecipients if checkRecipientAllowed(recipient, domains, ssContent)]
            validRecipients = toRecipients + ccRecipients + bccRecipients
    else:
        validRecipients = recipients

    setEmailRecipientField(email, 'To', toRecipients)
    setEmailRecipientField(email, 'Cc', ccRecipients)

    mail_log_msg = 'Sending email. sid=%s, subject="%s", encoded_subject="%s", results_link="%s", recipients="%s", server="%s", to="%s", cc="%s"' % (
        sid,
        ...
        str(validRecipients),
        str(server),
        email.get('To') if email.get('To') is not None else '',
        email.get('Cc') if email.get('Cc') is not None else ''
    )
    try:
